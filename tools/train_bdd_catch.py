...
def setup(args):
    ...
    cfg.OUTPUT_DIR = './out/CondInst_R_101_3x_sem_bdd100k_single_scale'
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if not torch.isfinite(losses).all():
                print('!!!infinite loss batch catched!!!')
                torch.save(model.state_dict(), os.path.join(cfg.OUTPUT_DIR, 'catched_cpt.pth'))
                torch.save(data, os.path.join(cfg.OUTPUT_DIR, 'catched_data.pth'))
                exit(0)
